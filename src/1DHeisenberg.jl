struct LatticeModel{AT<:AbstractLattice{D}, D}
    lattice::AT # lattice type
    N::Int   # system size
    pbc::Bool # periodic boundary conditions
    function LatticeModel(lattice::AT, N::Int; pbc::Bool=true) where {AT<:AbstractLattice{D}, D}
        @assert N > 0 "N is expected to be greater than 0, but got $N"
        return new{AT}(lattice, N, pbc)
    end
end

function basis(::LatticeModel{AT}, T) where {AT<:ChainLattice}
    # Generate basis for PXP model, return both decimal and binary form, where we both consider PBC and OBC
    basis = collect(0:2^AT.N - 1)
